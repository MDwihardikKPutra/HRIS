...
import { Search, History, Calendar, CreditCard, ClipboardList, Clock } from "lucide-react";
import {
  leaveRequests, spds, purchases, vendorPayments,
  workPlans, workRealizations, getUserById,
  getStatusColor
} from "@/lib/data";
import { FilterBar, StatusBadge, AvatarInitial } from "@/components/DataTable";
    ...
    <div className="space-y-4 w-full animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 bg-white border border-slate-200 rounded-xl">
          ...
          <span className="text-[10px] font-bold text-slate-600 capitalize tracking-wide">Sistem Aktif</span>
        </div>
      </div>
            ...
            {categories.map((cat) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold transition-all whitespace-nowrap capitalize tracking-wide border ${
      ...
      {/* STANDARDIZED: Main Content Table */}
                  ...
                  <tr key={log.id} className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="py-2.5 px-4">
                      <div className="flex items-center gap-2.5">
                        <AvatarInitial name={log.user.name} size="sm" />
                        <div className="min-w-0">
                          <p className="font-bold text-slate-800 truncate leading-none mb-0.5">{log.user.name}</p>
                          <p className="text-[10px] text-slate-400 font-bold capitalize truncate">{log.user.role}</p>
                        </div>
                      </div>
                    </td>
                    <td className="py-2.5 px-4">
                    ...
                    <td className="py-2.5 px-4 hidden md:table-cell">
                      <span className={`inline-flex items-center px-1.5 py-0.5 rounded-md font-bold text-[10px] capitalize tracking-wide border cursor-default ${
