...
export default function ActivityLogPage() {
  ...
  return (
    <div className="space-y-4 w-full animate-in fade-in duration-500">
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-[0_1px_3px_rgba(0,0,0,0.02)]">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            ...
            <div className="relative group min-w-[320px]">
                ...
                <input
                    ...
                    className="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs focus:outline-none focus:border-indigo-400 focus:bg-white transition-all font-medium shadow-sm"
                />
            </div>
        </div>
      </div>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100 font-semibold text-slate-500 uppercase tracking-widest text-[10px]">
                <th className="text-left py-4 px-6">Waktu & Tanggal</th>
                <th className="text-left py-4 px-4">Personel</th>
                <th className="text-left py-4 px-4">Deskripsi Aktivitas</th>
                <th className="text-left py-4 px-4 hidden md:table-cell">Kategori</th>
                  ...
                  <tr key={log.id} className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="py-4 px-4">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-xl bg-indigo-50 border border-indigo-100 flex items-center justify-center text-[10px] font-bold text-indigo-600">
                          {log.user.initial}
                        </div>
                        <div className="min-w-0">
                            <p className="font-bold text-slate-800 truncate leading-none mb-1">{log.user.name}</p>
                            <p className="text-[10px] text-slate-400 font-medium truncate">{log.user.role}</p>
                        </div>
                      </div>
                    </td>
                    <td className="py-4 px-4">
                        <div className="flex items-center gap-2">
                            <div className="min-w-0">
                                <p className="font-bold text-slate-700 leading-none mb-1">{log.action}</p>
                                <p className="text-[10px] text-indigo-500 font-sans font-bold tracking-tight">#{log.resource}</p>
                            </div>
                            <ArrowRight className="w-3 h-3 text-slate-200 group-hover:translate-x-1 transition-transform" />
                        </div>
                    </td>
                    <td className="py-4 px-4 hidden md:table-cell">
                        <span className={`inline-flex items-center px-2 py-1 rounded-md font-bold text-[12px] uppercase tracking-wider border cursor-default ${
                    ...
                    <td className="py-4 px-6 text-center">
                      <span className={`inline-flex px-2 py-1 rounded-lg text-[12px] font-black border uppercase tracking-widest ${sc.text} ${sc.border === 'border-slate-200' ? 'border-slate-300' : sc.border}`}>
                        {log.status}
                      </span>
                    </td>
                  </tr>
